...
function TwitchEmotesAnimator_OnUpdate(self, elapsed)
	...
	if (TWITCHEMOTES_TimeSinceLastUpdate >= 0.033) then
		...
		if(TwitchStatsScreen:IsVisible()) then
			...
			local topSentImagePath = TwitchEmotes_defaultpack[TwitchEmoteSentStatKeys[1]] or "Interface\\AddOns\\LizzieEmotes\\Emotes\\1337.tga";
			local animdata = TwitchEmotes_animation_metadata[topSentImagePath:match("(Interface\\AddOns\\LizzieEmotes\\Emotes.-.tga)")]
			...
			local topSeenImagePath = TwitchEmotes_defaultpack[TwitchEmoteRecievedStatKeys[1]] or "Interface\\AddOns\\LizzieEmotes\\Emotes\\1337.tga";
			local animdata = TwitchEmotes_animation_metadata[topSeenImagePath:match("(Interface\\AddOns\\LizzieEmotes\\Emotes.-.tga)")]
			if(animdata ~= nil) then
				local cFrame = TwitchEmotes_GetCurrentFrameNum(animdata)
...
function TwitchEmotesAnimator_UpdateEmoteInFontString(fontstring, widthOverride, heightOverride)
	...
	if (txt ~= nil) then
		for emoteTextureString in txt:gmatch("(|TInterface\\AddOns\\LizzieEmotes\\Emotes.-|t)") do
			local imagepath = emoteTextureString:match("|T(Interface\\AddOns\\LizzieEmotes\\Emotes.-.tga).-|t")
